 ...
 * @author akshos
 */
public class BarrelAddIssueLift extends javax.swing.JInternalFrame implements RefreshOption{
     ...
     */
    public BarrelAddIssueLift() {
        initComponents();
    }
    public BarrelAddIssueLift(DBConnection db, int mode, String id, Main frame, int level){
        this.level = level;
    ...
    private void setDifference(){
        int stock, issued, lifted, difference;
        try{
            stock = Integer.parseInt(this.stockTbox.getText());
        }catch(Exception e){
            System.out.println("Resetting stock to 0");
            stock = 0;
        }
        try{
        ...
        difference = stock + issued - lifted;
    ...
    private void insertData(){
        ...
        if(mode==Codes.EDIT){
            ret = BarrelDB.update(stmt, this.editId, branch, party, date,String.valueOf(stock),String.valueOf(issued),String.valueOf(lifted),String.valueOf(difference));
            if(ret){
            ...
            ret = BarrelDB.insert(stmt, branch, party, date,String.valueOf(stock),String.valueOf(issued),String.valueOf(lifted),String.valueOf(difference));
            if(ret){
        ...
        //update the barrel balance of the customer
        CustomerDB.updateCustomerBarrelBalance(dbConnection.getStatement(), party);
    ...
    private void initComponents() {
        ...
        stockTbox.setFormatterFactory(new javax.swing.text.DefaultFormatterFactory(new javax.swing.text.NumberFormatter(new java.text.DecimalFormat("#0"))));
